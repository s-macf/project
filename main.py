...
def check_collisions(player, rects, velocity):
    ...
    player_rect = pygame.FRect((player[0] + velocity[0], player[1] + velocity[1]), (50, 50))
    for rect in rects:
    ...
    if sum(collisions) == 1:
        ...
        player_rect = basic_collision(player_rect, velocity, collisions, collided_floor, collided_ceiling, collided_walls)
...
def run():
    ...
    current_lvl = int((player[1] + 25) / 900) # Center of player determines level
    offset = current_lvl * (-screen.height)

    player_velocity = [0, 0]
    map_image, map_rects = load_env("./Jump-King-Tiled/jump-king-map.tmx")
    clock = pygame.Clock()
    ...
    while running:
        ...
        if current_lvl > 42: current_lvl = 42
        offset = current_lvl * (-screen.height)
        ...
        player, player_velocity = check_collisions(player, map_rects, player_velocity)
        player = [player[0] + player_velocity[0], player[1] + player_velocity[1]]
        pygame.draw.rect(screen, "blue", pygame.FRect((player[0], player[1] + offset), (50, 50)))
